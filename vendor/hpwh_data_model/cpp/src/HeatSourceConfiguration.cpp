...
namespace hpwh_data_model  {
	...
	namespace heat_source_configuration  {
		...
		void to_json(nlohmann::json& j, const WeightedDistribution& x) {
			json_set<std::vector<double>>(j, logger.get(), "normalized_height", x.normalized_height, x.normalized_height_is_set, true);
			json_set<std::vector<double>>(j, logger.get(), "weight", x.weight, x.weight_is_set, true);
		}
		...
		void to_json(nlohmann::json& j, const HeatingLogic& x) {
			json_set<heat_source_configuration::HeatingLogicType>(j, logger.get(), "heating_logic_type", x.heating_logic_type, x.heating_logic_type_is_set, true);
			if (x.heating_logic_type == heat_source_configuration::HeatingLogicType::STATE_OF_CHARGE_BASED) {
			...
			json_set<heat_source_configuration::ComparisonType>(j, logger.get(), "comparison_type", x.comparison_type, x.comparison_type_is_set, true);
		}
		...
		void to_json(nlohmann::json& j, const HeatSourceConfiguration& x) {
			json_set<std::string>(j, logger.get(), "id", x.id, x.id_is_set, true);
			json_set<heat_source_configuration::HeatSourceType>(j, logger.get(), "heat_source_type", x.heat_source_type, x.heat_source_type_is_set, true);
			if (x.heat_source_type == heat_source_configuration::HeatSourceType::RESISTANCE) {
			...
			json_set<heat_source_configuration::WeightedDistribution>(j, logger.get(), "heat_distribution", x.heat_distribution, x.heat_distribution_is_set, true);
			json_set<std::vector<heat_source_configuration::HeatingLogic>>(j, logger.get(), "turn_on_logic", x.turn_on_logic, x.turn_on_logic_is_set, true);
			json_set<std::vector<heat_source_configuration::HeatingLogic>>(j, logger.get(), "shut_off_logic", x.shut_off_logic, x.shut_off_logic_is_set, false);
			json_set<heat_source_configuration::HeatingLogic>(j, logger.get(), "standby_logic", x.standby_logic, x.standby_logic_is_set, true);
			json_set<std::string>(j, logger.get(), "backup_heat_source_id", x.backup_heat_source_id, x.backup_heat_source_id_is_set, false);
			json_set<std::string>(j, logger.get(), "followed_by_heat_source_id", x.followed_by_heat_source_id, x.followed_by_heat_source_id_is_set, false);
			json_set<std::string>(j, logger.get(), "companion_heat_source_id", x.companion_heat_source_id, x.companion_heat_source_id_is_set, false);
		}
		...
		void to_json(nlohmann::json& j, const TemperatureBasedHeatingLogic& x) {
			json_set<double>(j, logger.get(), "absolute_temperature", x.absolute_temperature, x.absolute_temperature_is_set, true);
			json_set<double>(j, logger.get(), "differential_temperature", x.differential_temperature, x.differential_temperature_is_set, true);
			json_set<heat_source_configuration::StandbyTemperatureLocation>(j, logger.get(), "standby_temperature_location", x.standby_temperature_location, x.standby_temperature_location_is_set, true);
			json_set<heat_source_configuration::WeightedDistribution>(j, logger.get(), "temperature_weight_distribution", x.temperature_weight_distribution, x.temperature_weight_distribution_is_set, true);
		}
		...
		void to_json(nlohmann::json& j, const StateOfChargeBasedHeatingLogic& x) {
			json_set<double>(j, logger.get(), "decision_point", x.decision_point, x.decision_point_is_set, true);
			json_set<double>(j, logger.get(), "minimum_useful_temperature", x.minimum_useful_temperature, x.minimum_useful_temperature_is_set, false);
			json_set<double>(j, logger.get(), "hysteresis_fraction", x.hysteresis_fraction, x.hysteresis_fraction_is_set, false);
			json_set<bool>(j, logger.get(), "uses_constant_mains", x.uses_constant_mains, x.uses_constant_mains_is_set, true);
			json_set<double>(j, logger.get(), "constant_mains_temperature", x.constant_mains_temperature, x.constant_mains_temperature_is_set, true);
		}
