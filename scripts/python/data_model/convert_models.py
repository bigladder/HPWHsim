...
def convert_models(models_list_file, build_dir, output_dir):
    ...
    with open(models_list_file) as json_file:
        ...
        # app_cmd = os.path.join(abs_build_dir , 'src', 'hpwh', 'hpwh') #-nix build path
        app_cmd = os.path.join(abs_build_dir, "src", "hpwh", "Debug", "hpwh")  # Visual Studio build path
        if output_dir == "":
        ...
        for model, id in json_data.items():
            ...
            print(convert_list)
            subprocess.run(convert_list, stdout=subprocess.PIPE, text=True, check=False)
