...
class PerfProc:
    ...
    def plot(self, data):
        ...
        if self.plotter.have_data:
            ...
            return (
                self.plotter.fig,
                not (self.plotter.have_data),
                not (show_outletTs),
                self.plotter.iT3,
                outletTs,
                show_list,
                self.prefs["performance"]["plots"]["contour_variable"],
                self.prefs["performance"]["plots"]["contour_coloring"],
                data["interp_div"],
            )
    ...
    def proc(self, data):  # noqa: PLR0915
        ...
        app.layout = [
            ...
            html.Div(
                [
                    html.P(
                        ...
                        style={"fontSize": "12px", "margin": "4px", "display": "inline-block"},
                    ),
                    dcc.Dropdown(
                        id="outletT-dropdown",
                        style={"width": "50%", "display": "inline-block", "verticalAlign": "middle"},
            ...
            html.P("show"),
            dcc.Checklist(options=[{"label": "points", "value": "points"}], id="show-check"),
            html.Br(),
            html.Div(
                dcc.Graph(
                    id="perf-graph",
            ...
            html.Div(
                [
                    ...
                    html.Button(
                        "+",
                        id="add-selected-to-marked",
                        n_clicks=0,
                        style={"fontSize": "12px", "margin": "1px", "display": "inline-block"},
                    ),
                    html.Button(
                        "-",
                        id="remove-selected-from-marked",
                        n_clicks=0,
                        style={"fontSize": "12px", "margin": "1px", "display": "inline-block"},
                    ),
                    html.Button(
                    ...
                    html.Div(
                        [
                            dcc.Checklist(
                                options=[{"label": "interpolate", "value": "interpolate"}], id="interp-check"
                            ),
                            html.Div(
                                [
        ...
        def send_message(n_clicks):
            ...
            message = json.dumps({"source": "perf-proc", "dest": "index", "cmd": "init", "index": self.i_send})
            return message
                    ...
                    if msg_data["dest"] == "perf-proc":
                        ...
                        if "cmd" in msg_data:
                            if msg_data["cmd"] == "plot":
                                return self.plot(
                                    {"model_filepath": msg_data["model_filepath"], "interp_div": interp_div}
                                )
        ...
        @app.callback(
            Output("ws", "send", allow_duplicate=True), Input("vary-marked", "n_clicks"), prevent_initial_call=True
        )
        ...
        @app.callback(
            Output("ws", "send", allow_duplicate=True), Input("hold-marked", "n_clicks"), prevent_initial_call=True
        )
