# uv run --no-project test_plot.py "/../../HPWHsim/test/Villara/AquaThermAire/villara_24hr67/measured.csv" "../../HPWHsim/build/test/output/villara_24hr67__AquaThermAire.csv" :../../HPWHsim/build/test/output/villara_24hr67__AquaThermAire.html"
...
class Plotter:
    def __init__(self):
        ...
        self.variables = {
            "Y-Variables": {
                ...
                "Temperature": {
                    "Column Names": {
                        "Measured": [f"TankT{number}(C)" for number in range(1, NUMBER_OF_THERMOCOUPLES + 1)],
                        "Simulated": [
        ...
        for variable_type in ["Measured", "Simulated"]:
            for index, label in enumerate(INPUT_TEMPERATURE_DETAILS[variable_type]):
                self.variables["Y-Variables"]["Temperature"]["Column Names"][variable_type].insert(index, label)
    ...
    def filter_dataframe_range(self, df, variable_type):
        ...
        return df[(df[column_time_name] > 0) & (df[column_time_name] <= 1440)].reset_index()  # noqa: PLR2004
    ...
    def organize_tank_temperatures(self, variable_type):
        ...
        df["Storage Tank Average Temperature"] = df[self.temperature_profile_vars[variable_type]].mean(axis=1)

        for temperature_column in self.variables["Y-Variables"]["Temperature"]["Column Names"][variable_type]:
            for index in range(len(df)):
                df.loc[index, temperature_column] = (
                    1.8 * df.loc[index, temperature_column] + 32
                )  # convert(df.loc[index, temperature_column], "degC", "degF")
        ...
        while src_exists:
            ...
            if src_exists:
                if i == 1:
                    ...
                    self.df_simulated[power_col_label_sim] = (
                        self.df_simulated[power_col_label_sim] + self.df_simulated[col_label]
                    )
            i = i + 1
    ...
    def draw_variable_type(self, variable_type):
        for row, variable in enumerate(self.variables["Y-Variables"].keys()):
            for value in range(len(self.variables["Y-Variables"][variable]["Column Names"][variable_type])):
