...
uv run test_plot.py \
	...
	"villara_24hr67" \
	"" \
	"/Users/.../Documents/GitHub/HPWHsim/build/test/output/villara_24hr67_JSON_BradfordWhiteAeroThermRE2HP50.csv" \
	"temp.html"
"""
...
from common import get_tank_volume, read_file
...
class TestPlotter:
        ...
        self.variables = {
            "Y-Variables": {
                ...
                "Temperature": {
                    ...
                    "Colors": ["#e1141e", "#53a05d", "#6e62a4", "#b05593", "#cbc062"]
                    + list(reversed(RED_BLUE_DIVERGING_PALLETTE)),
                    "Line Mode": {"Tank": ["lines"] * (4 + NUMBER_OF_THERMOCOUPLES)},
                    "Line Visibility": [False] * 4
                    + [
                        True if i == 0 or i == NUMBER_OF_THERMOCOUPLES - 1 else False
    ...
    def draw(self):
        for dataset in self.datasets:
            ...
            self.plot = dimes.DimensionalPlot(
                [x for x in dataset.df["Time"]], f"Model: {dataset.model_id}, Test: {dataset.test_id}"
            )
    ...
    def select_data(self, selectedData):  # noqa: PLR0912
        self.test_points = []
        result = []
        if "points" in selectedData:
            curve_dict = {}
            for point in selectedData["points"]:
                if point["curveNumber"] not in curve_dict:
                    curve_dict[point["curveNumber"]] = self.plot.figure["data"][point["curveNumber"]]

            curveSums = {}
            for curveNumber, curve in curve_dict.items():
                if "name" in curve:
                    for dataset in self.datasets:
            ...
            result = [[curveSum[2], curveSum[3] * curveSum[1]] for curveSum in curveSums.values()]
    ...
    def update_graphs(self, dataset, variable, value, row):
        self.plot.figure.update_traces(
            y=[x for x in dataset.df[self.variables["Y-Variables"][variable]["Columns"][value][row]]],
            ...
            if "point_number" in self.click_point:
                for curve in self.plot.figure["data"]:
                    if curve["name"] == self.click_point["curve_name"]:
                        ...
                        self.plot.figure.update_traces(x=xc, y=yc, marker_size=20, selector=dict(name="clicked points"))
            ...
            for dataset_spec in data["dataset_specs"]:
                for dataset in self.datasets:
                    if dataset_spec["filepath"] == dataset.filepath:
                        self.update_dataset(DataSet(dataset_spec))
                    else:
                ...
                if dataset.traces_hidden:
                    ...
                    for curve in self.plot.figure["data"]:
                        if curve["name"] in dataset.trace_visible:
                            self.plot.figure.update_traces(
                                visible=dataset.trace_visible[curve["name"]], selector=dict(name=curve["name"])
                            )
            elif not dataset.traces_hidden:
                dataset.traces_hidden = True
                for curve in self.plot.figure["data"]:
                    if curve["name"] in dataset.trace_visible:
                        dataset.trace_visible[curve["name"]] = curve["visible"]
                        self.plot.figure.update_traces(visible=False, selector=dict(name=curve["name"]))
    ...
    def getSummaryDataDict(self):
        ...
        for dataset in self.datasets:
            for item, dict_data in summary_data_dict.items():
                have_item = False
                for summary in ["first-recovery-period", "standby-period", "24-hr-test"]:
                    if summary in dataset.test_summary:
                        if item in dataset.test_summary[summary]:
                            have_item = True
                            dict_data.append(dataset.test_summary[summary][item])
                            break
                if not (have_item):
                    dict_data.append("")
...
if __name__ == "__main__":
    ...
    if n_args > 3:  # noqa: PLR2004
        model_id = sys.argv[1]
        test_id = sys.argv[2]
        data["dataset_specs"] = []

        if sys.argv[3] != "":
            data["dataset_specs"].append(
                {"model_id": model_id, "test_id": test_id, "type": "Measured", "filepath": sys.argv[3]}
            )
        if sys.argv[4] != "":
            data["dataset_specs"].append(
                {"model_id": model_id, "test_id": test_id, "type": "Simulated", "filepath": sys.argv[4]}
            )
    if n_args > 4:  # noqa: PLR2004
        data["plot_filepath"] = sys.argv[5]
    if n_args > 5:  # noqa: PLR2004
