...
from common import get_perf_map
...
class PerfPlotter:
    ...
    def update_marks(self, value, prefs):
        for iT1, T1 in enumerate(self.T1s):
            for iT2, T2 in enumerate(self.T2s):
                this_mark = self.marked[iT1, iT2, self.iT3] & (1 << prefs["performance"]["plots"]["contour_variable"])
                other_marks = self.marked[iT1, iT2, self.iT3] & (~this_mark)
                if (
                    self.selected[iT1, iT2]
                    and not self.dependent[iT1, iT2, self.iT3] == prefs["performance"]["plots"]["contour_variable"]
                ):
                    this_mark = value * (1 << prefs["performance"]["plots"]["contour_variable"])
    ...
    def interpolate(self, refs, prefs):
        ...
        for iT1, T1 in enumerate(self.T1s):
            for iT2, T2 in enumerate(self.T2s):
                if self.selected[iT1, iT2]:
                    iT1_min = min(iT1_min, iT1)
                    iT2_min = min(iT2_min, iT2)
                    iT1_max = min(iT1_max, iT1)
                    iT2_max = min(iT2_max, iT2)
        ...
        nX = prefs["performance"]["plots"]["Nx"]
        nY = prefs["performance"]["plots"]["Ny"]
    ...
    def draw(self, prefs):  # noqa: PLR0915
        ...
        if "contour_variable" in prefs["performance"]["plots"]:
        ...
        is_interp = ("interpolate" in prefs["performance"]["plots"]) and (
            prefs["performance"]["plots"]["interpolate"] == 1
        )
        if is_interp:
        ...
        lineWidth = 0
        showLabels = False
        if "contour_coloring" in prefs["performance"]["plots"]:
            if prefs["performance"]["plots"]["contour_coloring"] == 1:  # noqa: PLR2004
                coloring = "heatmap"
                lineWidth = 2
                showLabels = True
            if prefs["performance"]["plots"]["contour_coloring"] == 2:  # noqa: PLR2004
                coloring = "lines"
                lineWidth = 2
                showLabels = True
        self.fig = go.Figure(
        ...
        points_visible = ("show_points" in prefs["performance"]["plots"]) and (
            prefs["performance"]["plots"]["show_points"] == 1
        )
        ...
        self.fig.update_layout(xaxis_title=x_title, yaxis_title=y_title, title=graph_title, title_x=0.5)
    ...
    def update_markers(self, prefs):
        ...
        x_label = "Tenv (\u00b0C)"
        y_label = "Tinlet (\u00b0C)" if self.is_central else "Tcond (\u00b0C)"
        value_label = self.variables[prefs["performance"]["plots"]["contour_variable"]]
    ...
    def update_dependent(self, prefs):
        ...
        for iT2, T2 in enumerate(self.refs[1]):
            for iT1, T1 in enumerate(self.refs[0]):
                ...
                if depends == prefs["performance"]["plots"]["contour_variable"]:
        ...
        for iT2, T2 in enumerate(self.refs[1]):
            for iT1, T1 in enumerate(self.refs[0]):
                ...
                if marks > 0:
                    ...
                    if (marks & (1 << prefs["performance"]["plots"]["contour_variable"])) > 0:
    ...
    def get_marked_list(self, prefs):
        ...
        if self.is_central:
            for iT2, T2 in enumerate(self.refs[1]):
                for iT1, T1 in enumerate(self.refs[0]):
                    for i in range(3):
                        if (self.marked[iT1, iT2, self.iT3] & (1 << i)) & (
                            1 << prefs["performance"]["plots"]["contour_variable"]
                        ) > 0:
