...
if format == "Villara":
    orig_columns = dict(
        [("Time", 0), ("AmbientT", 14), ("Power", 2), ("TankT1", 3), ("InletT", 12), ("OutletT", 13), ("Draw", 10)]
    )
    power_factor = 1000.0
if format == "BradfordWhite":
    orig_columns = dict(
        [("Time", 1), ("AmbientT", 2), ("Power", 5), ("TankT1", 6), ("InletT", 12), ("OutletT", 13), ("Draw", 14)]
    )
    power_factor = 1.0
if format == "LG":
    orig_columns = dict(
        [("Time", 0), ("AmbientT", 14), ("Power", 2), ("TankT1", 8), ("InletT", 12), ("OutletT", 13), ("Draw", 10)]
    )
    power_factor = 1000.0
            ...
            if nLines >= numRowsPerMin:
                if flowRate_sum > 0:
                    if iMin >= initTime_min:
                        out_file.writelines(
                            "{min}, {flowRate_avg:.3f}\n".format(
                                min=iMin - initTime_min, flowRate_avg=flowRate_sum / nLines
                            )
                        )
                nLines = 0
                flowRate_sum = 0
        ...
        if not first:
            ...
            if nLines >= numRowsPerMin:
                if iMin >= initTime_min:
                    out_file.writelines(
                        "{min}, {ambientT_avg:.3f}\n".format(
                            min=iMin - initTime_min, ambientT_avg=ambientT_sum / nLines
                        )
                    )
                ambientT_sum = 0
                nLines = 0
...
# ruff: noqa: PLR2004
# test info
def create_test_into(test_dir, data_filename):  # noqa: PLR0912, PLR0915
    in_filename = str(data_filename) + ".csv"
    in_file_path = os.path.join(test_dir, in_filename)
    in_file = open(in_file_path, "r")
    Lines = in_file.readlines()
...
# measured data
def convert_measured(test_dir, data_filename):  # noqa: PLR0912, PLR0915
    in_filename = str(data_filename) + ".csv"
    in_file_path = os.path.join(test_dir, in_filename)
    in_file = open(in_file_path, "r")
    Lines = in_file.readlines()
