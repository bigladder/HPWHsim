	...
	namespace rsintegratedwaterheater_ns  {
		...
		void from_json(const nlohmann::json& j, HeatingLogic& x) {
			...
			if (x.heating_logic_type == rsintegratedwaterheater_ns::HeatingLogicType::STATE_OF_CHARGE_BASED) {
				x.heating_logic = std::make_unique<rsintegratedwaterheater_ns::StateOfChargeBasedHeatingLogic>();
				if (x.heating_logic) {
					from_json(j.at("heating_logic"), *dynamic_cast<rsintegratedwaterheater_ns::StateOfChargeBasedHeatingLogic*>(x.heating_logic.get()));
				}
			}
			if (x.heating_logic_type == rsintegratedwaterheater_ns::HeatingLogicType::TEMPERATURE_BASED) {
				x.heating_logic = std::make_unique<rsintegratedwaterheater_ns::TemperatureBasedHeatingLogic>();
				if (x.heating_logic) {
					from_json(j.at("heating_logic"), *dynamic_cast<rsintegratedwaterheater_ns::TemperatureBasedHeatingLogic*>(x.heating_logic.get()));
				}
			}
		...
		void from_json(const nlohmann::json& j, Performance& x) {
			...
			json_get<bool>(j, logger.get(), "fixed_setpoint", x.fixed_setpoint, x.fixed_setpoint_is_set, false);
		}
		...
		const std::string_view TemperatureBasedHeatingLogic::tank_node_specification_description = "Tank-node specification";
		...
		const std::string_view TemperatureBasedHeatingLogic::tank_node_specification_name = "tank_node_specification";
