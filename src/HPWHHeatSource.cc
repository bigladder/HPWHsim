    ...
    hpwh_data_model::heat_source_configuration::HeatSourceConfiguration& hsc) const
{
    ...
    {
        heights.push_back(heatDist.fractionalHeight(i));
        weights.push_back(heatDist.fractionalWeight(i));
    }
...
double HPWH::HeatSource::heat(double cap_kJ, const double maxSetpointT_C)
{
    ...
    for (int i = numNodes - 1; i >= 0; i--)
    {
        ...
        double frac = heatDistribution[i] / maxWeight; // heatDistribution[i] > 0. ? 1. : 0.; //
        double withheldCap_kJ = std::pow(1. - frac, withhold_pow) /*(1. - frac)*/ * leftoverCap_kJ;
