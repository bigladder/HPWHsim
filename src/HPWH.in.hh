...
class HPWH
{
  public:
    ...
    static const int CONDENSITY_SIZE =
        12; /**< this must be an integer, and only the value 12 change at your own risk */
   ...
   simulation when the location temperature has not been initialized */
    ...
    struct TempBasedHeatingLogic : HeatingLogic
    {
      public:
        TempBasedHeatingLogic(std::string desc,
                              ...
                              std::function<bool(double, double)> c = std::less<double>(),
                              bool isHTS = false)
            : HeatingLogic(desc, d, phpwh, c, isHTS), nodeWeights(n), isAbsolute(a) {};
    ...
    bottomNodeMaxTemp(double d, bool isEnteringWaterHighTempShutoff = false);
    ...
    regardless of their order in setOfSources. If the elements exist on at the same node then all of
  ...
  private:
    ...
    void updateTankTemps(
        double draw, double inletT, double ambientT, double inletVol2_L, double inletT2_L);
    ...
    /**< adds extra heat defined by the user. Where nodeExtraHeat[] is a vector of heat quantities
     * to be added during the step.  nodeExtraHeat[ 0] would go to bottom node, 1 to next etc.  */
    ...
    resistance elements to their indexes in setOfSources. */
    ...
    /**< an array holding the future temperature of each node for the conduction calculation - 0 is
     * the bottom node, numNodes is the top  */
...
class HPWH::HeatSource
{
  public:
    ...
    void setCondensity(double cnd1,
                       double cnd2,
                       double cnd3,
                       double cnd4,
                       double cnd5,
