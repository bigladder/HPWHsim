...
namespace data_model  {
	...
	namespace ashrae205_ns  {
		...
		void from_json(const nlohmann::json& j, LiquidComponent& x) {
			json_get<ashrae205_ns::LiquidConstituent>(j, logger.get(), "liquid_constituent", x.liquid_constituent, x.liquid_constituent_is_set, true);
		...
		void from_json(const nlohmann::json& j, LiquidMixture& x) {
			json_get<std::vector<ashrae205_ns::LiquidComponent>>(j, logger.get(), "liquid_components", x.liquid_components, x.liquid_components_is_set, true);
			json_get<ashrae205_ns::ConcentrationType>(j, logger.get(), "concentration_type", x.concentration_type, x.concentration_type_is_set, true);
		}
