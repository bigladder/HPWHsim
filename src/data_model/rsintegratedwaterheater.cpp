...
namespace hpwh_data_model  {
	...
	namespace rsintegratedwaterheater_ns  {
		...
		void from_json(const nlohmann::json& j, HeatingLogic& x) {
			json_get<rsintegratedwaterheater_ns::HeatingLogicType>(j, "heating_logic_type", x.heating_logic_type, x.heating_logic_type_is_set, true);
			...
			json_get<rsintegratedwaterheater_ns::ComparisonType>(j, "comparison_type", x.comparison_type, x.comparison_type_is_set, true);
		}
		...
		void from_json(const nlohmann::json& j, HeatSourceConfiguration& x) {
			...
			json_get<std::vector<rsintegratedwaterheater_ns::HeatingLogic>>(j, "shut_off_logic", x.shut_off_logic, x.shut_off_logic_is_set, false);
			json_get<rsintegratedwaterheater_ns::HeatingLogic>(j, "standby_logic", x.standby_logic, x.standby_logic_is_set, false);
			json_get<double>(j, "maximum_setpoint", x.maximum_setpoint, x.maximum_setpoint_is_set, false);
			json_get<double>(j, "hysteresis_temperature_difference", x.hysteresis_temperature_difference, x.hysteresis_temperature_difference_is_set, false);
			json_get<bool>(j, "is_vip", x.is_vip, x.is_vip_is_set, false);
			json_get<bool>(j, "depresses_temperature", x.depresses_temperature, x.depresses_temperature_is_set, false);
			json_get<std::string>(j, "backup_heat_source_label", x.backup_heat_source_label, x.backup_heat_source_label_is_set, false);
			json_get<std::string>(j, "followed_by_heat_source_label", x.followed_by_heat_source_label, x.followed_by_heat_source_label_is_set, false);
			json_get<std::string>(j, "companion_heat_source_label", x.companion_heat_source_label, x.companion_heat_source_label_is_set, false);
		}
		...
		void from_json(const nlohmann::json& j, SoCBasedHeatingLogic& x) {
			...
			json_get<double>(j, "minimum_useful_temperature", x.minimum_useful_temperature, x.minimum_useful_temperature_is_set, false);
			json_get<double>(j, "hysteresis_fraction", x.hysteresis_fraction, x.hysteresis_fraction_is_set, false);
			json_get<bool>(j, "uses_constant_mains", x.uses_constant_mains, x.uses_constant_mains_is_set, true);
			json_get<double>(j, "constant_mains_temperature", x.constant_mains_temperature, x.constant_mains_temperature_is_set, true);
		}
