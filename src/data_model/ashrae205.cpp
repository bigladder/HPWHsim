...
namespace hpwh_data_model  {
	...
	namespace ashrae205_ns  {
		...
		void from_json(const nlohmann::json& j, LiquidComponent& x) {
			json_get<ashrae205_ns::LiquidConstituent>(j, "liquid_constituent", x.liquid_constituent, x.liquid_constituent_is_set, true);
			json_get<double>(j, "concentration", x.concentration, x.concentration_is_set, false);
		}
		...
		void from_json(const nlohmann::json& j, LiquidMixture& x) {
			json_get<std::vector<ashrae205_ns::LiquidComponent>>(j, "liquid_components", x.liquid_components, x.liquid_components_is_set, true);
			json_get<ashrae205_ns::ConcentrationType>(j, "concentration_type", x.concentration_type, x.concentration_type_is_set, true);
		}
