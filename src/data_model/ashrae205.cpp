...
namespace hpwh_data_model  {
	...
	namespace ashrae205_ns  {
        ...
        std::shared_ptr<Courier::Courier> ASHRAE205::logger {};

		void from_json(const nlohmann::json& j, LiquidComponent& x) {
			json_get<ashrae205_ns::LiquidConstituent>(j, *ASHRAE205::logger, "liquid_constituent", x.liquid_constituent, x.liquid_constituent_is_set, true);
			json_get<double>(j, *ASHRAE205::logger, "concentration", x.concentration, x.concentration_is_set, false);
		}
		...
		void from_json(const nlohmann::json& j, LiquidMixture& x) {
			json_get<std::vector<ashrae205_ns::LiquidComponent>>(j, *ASHRAE205::logger, "liquid_components", x.liquid_components, x.liquid_components_is_set, true);
			json_get<ashrae205_ns::ConcentrationType>(j, *ASHRAE205::logger, "concentration_type", x.concentration_type, x.concentration_type_is_set, true);
		}
