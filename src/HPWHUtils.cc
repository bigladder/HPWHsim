...
static void generate_metadata(RSTYPE& rs)
{
    auto& metadata
    checkTo({"Big Ladder Software, LLC"}, rs.)
    j["schema_author"] = "Big Ladder Software, LLC";
    j["author"] = "California Energy Commission";
    j["description"] = "Representation of embedded HPWH element";
    j["source"] = "Generated by HPWHsim";
...
/*static*/
template <typename T>
void product_information_to_json(const T& rs, nlohmann::json& j)
{
    if (rs.description_is_set)
    {
        ...
        if (rs.description.product_information_is_set)
        {
            ...
            if (prod_info.manufacturer_is_set)
                j["description"]["product_information"]["manufacturer"] = prod_info.manufacturer;
            if (prod_info.model_number_is_set)
                j["description"]["product_information"]["model_number"] = prod_info.model_number;
    }}
}
