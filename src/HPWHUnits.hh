...
} inline kJ_per_hC = UA::kJ_per_hC, Btu_per_hF = UA::Btu_per_hF;
...
template <>
inline Scaler<Temp>::ScaleMap Scaler<Temp>::scaleMap(C, {{F, F_per_C}});

template <>
inline ScaleOffseter<Temp>::ScaleOffsetMap
    ScaleOffseter<Temp>::scaleOffsetMap(C, {{F, {F_per_C, offsetF}}});
...
inline Scaler<UA>::ScaleMap Scaler<UA>::scaleMap(kJ_per_hC,
                                                 {{Btu_per_hF, scale(kJ, Btu) * scale(C, F)}});
...
template <Temp units>
using dTempVal = ScaleVal<Temp, units>;
