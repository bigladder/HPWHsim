                                           ...
                                           double& cop)
{
    ...
    do
    {
        ...
        // mix the tank
        double mixFactor = mpFlowRate_LPS * (timeRemaining_min * sec_per_min) / hpwh->nodeVolume_L;
        if (mixFactor > 1.)
        {
            mixFactor = 1.;
        }
        hpwh->mixTankNodes(
            0, hpwh->getNumNodes(), mixFactor); // 1.0 will give even mixing, so all temperatures
