...
double getCapacityMP_F_KW(HPWH& hpwh, performancePointMP& point)
{
    return hpwh.getCompressorCapacity(
        point.tairF, point.tinF, point.tinF, HPWH::P_UNITS::KW, HPWH::T_UNITS::F);
}
...
{
    return hpwh.getCompressorCapacity(
        point.tairF, point.tinF, point.toutF, HPWH::P_UNITS::BTUperH, HPWH::T_UNITS::F);
}
...
void testQAHVMatchesDataMap()
{
    ...
    ASSERTTRUE(getCapacitySP_F_BTUHR(hpwh, checkPoint) ==
               HPWH::HPWH_ABORT); // max setpoint without adjustment forces error
    ASSERTTRUE(
        relcmpd(checkPoint.outputBTUH,
...
void testQAHVExtrapolates()
{
    ...
    ASSERTTRUE(checkPoint.outputBTUH >
