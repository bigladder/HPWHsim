...
void testGetStateOfCharge() {
	...
	// Check for errors	
	chargeFraction = hpwh.calcSoCFraction(F_TO_C(125.), tMinUseful_C);
	ASSERTTRUE(chargeFraction == HPWH::HPWH_ABORT);
	chargeFraction = hpwh.calcSoCFraction(tMains_C, F_TO_C(155.));
	ASSERTTRUE(chargeFraction == HPWH::HPWH_ABORT);
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C, F_TO_C(100.));
	ASSERTTRUE(chargeFraction == HPWH::HPWH_ABORT);

	// Check state of charge returns 1 at setpoint
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C);
	ASSERTTRUE(cmpd(chargeFraction, 1.));
	chargeFraction = hpwh.calcSoCFraction(tMains_C + 5., tMinUseful_C);
	ASSERTTRUE(cmpd(chargeFraction, 1.));
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C + 5.);
	ASSERTTRUE(cmpd(chargeFraction, 1.));

	// Varying max temp
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C, F_TO_C(110.));
	ASSERTTRUE(cmpd(chargeFraction, 1.70909));
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C, F_TO_C(120.));
	ASSERTTRUE(cmpd(chargeFraction, 1.4461));
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C, F_TO_C(135.));
...
void testChargeBelowSetpoint() {
	...
	chargeFraction = hpwh.calcSoCFraction(tMains_C, tMinUseful_C, F_TO_C(140.));
