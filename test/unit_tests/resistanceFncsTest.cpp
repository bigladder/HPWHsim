...
/*
enum class rFactor {
    m2C_per_W , // kilojoules per hour degree celsius
    ft2hF_per_BtuF // british thermal units per hour degree Fahrenheit
} inline kJ_per_hC = rFactor::m2C_per_W,
         ft2hF_per_BtuF = rFactor::ft2hF_per_BtuF;


template <>
inline Units::Scaler<rFactor>::ScaleMap
    Units::Scaler<rFactor>::scaleMap(kJ_per_hC,
                         {{Btu_per_hF, scale(kJ, Btu) *
                                           scale(C, F)}});

inline auto FT2HFperBTU_TO_M2CperW(const double x) { using namespace Units; return scale(kJ_per_hC,
ft2hF_per_Btu) * x;  }
*/
...
{
    {
        ...
        EXPECT_EQ(hpwh.setResistanceCapacity(100.), HPWH::HPWH_ABORT); // Needs to be scalable
    }
...
TEST(ResistanceFunctionsTest, getSetResistanceErrors)
{
    ...
    EXPECT_EQ(hpwh.initResistanceTank(100., 0.95, 0., lowerElementPower_W, Units::L, Units::W), 0)
        << "Could not initialize resistance tank.";

    double returnVal;

    returnVal = hpwh.getResistanceCapacity(0, Units::W); // lower
    EXPECT_NEAR_REL(returnVal, lowerElementPower_W);

    returnVal = hpwh.getResistanceCapacity(-1, Units::W); // both,
    EXPECT_NEAR_REL(returnVal, lowerElementPower_W);

    returnVal = hpwh.getResistanceCapacity(1, Units::W); // higher doesn't exist
...
TEST(ResistanceFunctionsTest, commercialTankInitErrors)
{
    ...
    // init model
    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  -800., 10., 100., 100., Units::L, Units::m2, Units::C, Units::W),
    ...
    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  800., 0., 100., 100., Units::L, Units::m2, Units::C, Units::W),
              HPWH::HPWH_ABORT); // 0 r value

    EXPECT_EQ(
        hpwh.initResistanceTankGeneric(800., 10., 0., 0., Units::L, Units::m2, Units::C, Units::W),
...
TEST(ResistanceFunctionsTest, getNumResistanceElements)
{
    ...
    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  800., 10., 0., 1000., Units::L, Units::m2, Units::C, Units::W),
              0);
    EXPECT_EQ(hpwh.getNumResistanceElements(), 1); // Check 1 elements

    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  800., 10., 1000., 0., Units::L, Units::m2, Units::C, Units::W),
              0);
    EXPECT_EQ(hpwh.getNumResistanceElements(), 1); // Check 1 elements

    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  800., 10., 1000., 1000., Units::L, Units::m2, Units::C, Units::W),
              0);
...
TEST(ResistanceFunctionsTest, commercialTankErrorsWithBottomElement)
{
    ...
    HPWH hpwh;
    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  800., 10., 0., elementPower_kW * 1000., Units::L, Units::m2, Units::C, Units::W),
              0)
    ...
    EXPECT_EQ(hpwh.setResistanceCapacity(factor * elementPower_kW, -1, Units::kW),
              0); // Check sets
    EXPECT_NEAR_REL(hpwh.getResistanceCapacity(-1, Units::kW),
                    factor * elementPower_kW); // Check gets just bottom with both
    EXPECT_NEAR_REL(hpwh.getResistanceCapacity(0, Units::kW),
                    factor * elementPower_kW); // Check gets bottom with bottom
    EXPECT_EQ(hpwh.getResistanceCapacity(1, Units::kW),
              HPWH::HPWH_ABORT); // only have one element

    // set lowest
    factor = 4.;
    EXPECT_EQ(hpwh.setResistanceCapacity(factor * elementPower_kW, 0, Units::kW),
              0); // Set just bottom
    EXPECT_NEAR_REL(hpwh.getResistanceCapacity(-1, Units::kW),
                    factor * elementPower_kW); // Check gets just bottom with both
    EXPECT_NEAR_REL(hpwh.getResistanceCapacity(0, Units::kW),
...
TEST(ResistanceFunctionsTest, commercialTankErrorsWithTopElement)
{
    ...
    HPWH hpwh;
    EXPECT_EQ(hpwh.initResistanceTankGeneric(
                  800., 10., elementPower_kW * 1000., 0., Units::L, Units::m2, Units::C, Units::W),
              0)
        << "Could not initialize resistance tank.";
    ...
    EXPECT_EQ(hpwh.setResistanceCapacity(factor * elementPower_kW, 2, Units::kW), HPWH::HPWH_ABORT);
}
