...
const std::vector<string> noHighShuttOffVectMPExternal = {"Scalable_MP",
                                                          "ColmacCxA_20_MP",
                                                          "ColmacCxA_15_MP",
                                                          "ColmacCxV_5_MP",
                                                          "NyleC90A_MP",
...
const std::vector<string> noHighShuttOffVectIntegrated = {"AOSmithHPTU80",
                                                          "Rheem2020Build80",
                                                          "Stiebel220e",
                                                          "AOSmithCAHP120",
                                                          "AWHSTier3Generic80",
...
void testSetEnteringWaterShuffOffDeadbandToSmall(string& input)
{
    ...
    ASSERTTRUE(hpwh.setEnteringWaterHighTempShutOff(value, false, hpwh.getCompressorIndex()) ==
               HPWH::HPWH_ABORT);
...
/* Tests we can set the entering water high temp shutt off with aboslute values and it works turns
 * off and on as expected. */
void testSetEnteringWaterHighTempShutOffAbsolute(string& input)
{
...
void testSetEnteringWaterHighTempShutOffRelative(string& input)
{
    ...
    // change entering water temp to below temp
    ASSERTTRUE(hpwh.setEnteringWaterHighTempShutOff(
                   relativeHighT_C + delta, doAbsolute, hpwh.getCompressorIndex()) == 0);
...
void testChangeToStateofChargeControlled(string& input)
{
    ...
    ASSERTTRUE(hpwh.hasEnteringWaterHighTempShutOff(hpwh.getCompressorIndex()) ==
