...
void from_json(const nlohmann::json& j, GridVariables& x)
{
    json_get<std::vector<double>>(j,
                                  ...
                                  x.evaporator_environment_dry_bulb_temperature,
...
const std::string_view GridVariables::outlet_temperature_units = "K";

const std::string_view GridVariables::evaporator_environment_dry_bulb_temperature_description =
    "Dry bulb temperature of the air entering the evaporator coil";

const std::string_view GridVariables::heat_source_temperature_description =
    "Average water temperature at the heat source";

const std::string_view GridVariables::outlet_temperature_description =
    "Outlet temperature used by single-pass central water-heating systems";

const std::string_view GridVariables::evaporator_environment_dry_bulb_temperature_name =
    "evaporator_environment_dry_bulb_temperature";

const std::string_view GridVariables::heat_source_temperature_name = "heat_source_temperature";

const std::string_view GridVariables::outlet_temperature_name = "outlet_temperature";
...
void from_json(const nlohmann::json& j, LookupVariables& x)
{
    ...
    json_get<std::vector<double>>(
        j, logger.get(), "heating_capacity", x.heating_capacity, x.heating_capacity_is_set, true);
}
...
const std::string_view LookupVariables::input_power_description =
    "Power draw from the compressor, evaporator fan, and any auxiliary power used by the units "
    "controls";

const std::string_view LookupVariables::heating_capacity_description =
    "Total heat added by the condenser to the adjacent water";
...
const std::string_view PerformanceMap::grid_variables_description =
    "Data group defining the grid variables for heating performance";

const std::string_view PerformanceMap::lookup_variables_description =
    "Data group defining the lookup variables for heating performance";
...
void from_json(const nlohmann::json& j, Performance& x)
{
    ...
    json_get<double>(j,
                     ...
                     x.maximum_refrigerant_temperature,
                     x.maximum_refrigerant_temperature_is_set,
                     false);
    json_get<double>(j,
                     logger.get(),
                     "maximum_temperature",
                     x.maximum_temperature,
                     x.maximum_temperature_is_set,
                     false);
    json_get<double>(j,
                     logger.get(),
                     "minimum_temperature",
                     x.minimum_temperature,
                     x.minimum_temperature_is_set,
                     false);
    json_get<double>(j,
                     logger.get(),
                     "compressor_lockout_temperature_hysteresis",
                     x.compressor_lockout_temperature_hysteresis,
                     x.compressor_lockout_temperature_hysteresis_is_set,
                     false);
    json_get<bool>(
        j, logger.get(), "use_defrost_map", x.use_defrost_map, x.use_defrost_map_is_set, false);
    json_get<rscondenserwaterheatsource_ns::SecondaryHeatExchanger>(
        j,
        logger.get(),
        "secondary_heat_exchanger",
        x.secondary_heat_exchanger,
        x.secondary_heat_exchanger_is_set,
        false);
}
...
const std::string_view Performance::maximum_temperature_units = "K";

const std::string_view Performance::minimum_temperature_units = "K";
...
const std::string_view Performance::secondary_heat_exchanger_units = "";
...
const std::string_view Performance::maximum_refrigerant_temperature_description =
    "Maximum temperature of the refrigerant entering the condenser";

const std::string_view Performance::maximum_temperature_description =
    "Maximum external temperature";

const std::string_view Performance::minimum_temperature_description =
    "Minimum external temperature";

const std::string_view Performance::compressor_lockout_temperature_hysteresis_description =
    "Hysteresis for compressor lockout";

const std::string_view Performance::use_defrost_map_description = "Use defrost map";

const std::string_view Performance::secondary_heat_exchanger_description =
...
const std::string_view Performance::maximum_refrigerant_temperature_name =
    "maximum_refrigerant_temperature";

const std::string_view Performance::maximum_temperature_name = "maximum_temperature";

const std::string_view Performance::minimum_temperature_name = "minimum_temperature";

const std::string_view Performance::compressor_lockout_temperature_hysteresis_name =
    "compressor_lockout_temperature_hysteresis";

const std::string_view Performance::use_defrost_map_name = "use_defrost_map";

const std::string_view Performance::secondary_heat_exchanger_name = "secondary_heat_exchanger";
...
void from_json(const nlohmann::json& j, SecondaryHeatExchanger& x)
{
    json_get<double>(j,
                     ...
                     x.cold_side_temperature_offset,
                     x.cold_side_temperature_offset_is_set,
                     true);
    json_get<double>(j,
                     logger.get(),
                     "hot_side_temperature_offset",
                     x.hot_side_temperature_offset,
                     x.hot_side_temperature_offset_is_set,
                     true);
    json_get<double>(
        j, logger.get(), "extra_pump_power", x.extra_pump_power, x.extra_pump_power_is_set, true);
}
const std::string_view SecondaryHeatExchanger::cold_side_temperature_offset_units = "";

const std::string_view SecondaryHeatExchanger::hot_side_temperature_offset_units = "";

const std::string_view SecondaryHeatExchanger::extra_pump_power_units = "";

const std::string_view SecondaryHeatExchanger::cold_side_temperature_offset_description =
    "Cold-side offset temperature";

const std::string_view SecondaryHeatExchanger::hot_side_temperature_offset_description =
    "Hot-side offset temperature";

const std::string_view SecondaryHeatExchanger::extra_pump_power_description = "Extra pump power";

const std::string_view SecondaryHeatExchanger::cold_side_temperature_offset_name =
    "cold_side_temperature_offset";

const std::string_view SecondaryHeatExchanger::hot_side_temperature_offset_name =
    "hot_side_temperature_offset";
