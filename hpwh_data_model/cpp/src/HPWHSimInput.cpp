...
{
    json_get<int>(
        j, logger.get(), "number_of_nodes", x.number_of_nodes, x.number_of_nodes_is_set, false);
    json_get<bool>(j,
                   logger.get(),
    ...
    json_get<double>(j,
                     ...
                     false);
    json_get<hpwh_sim_input::HPWHSystemType>(
        j, logger.get(), "system_type", x.system_type, x.system_type_is_set, true);
    json_get<rsintegratedwaterheater::RSINTEGRATEDWATERHEATER>(j,
                                                               logger.get(),
                                                               "integrated_system",
                                                               x.integrated_system,
                                                               x.integrated_system_is_set,
                                                               true);
    json_get<central_water_heating_system::CentralWaterHeatingSystem>(
        j, logger.get(), "central_system", x.central_system, x.central_system_is_set, true);
}
const std::string_view HPWHSimInput::number_of_nodes_units = "";

const std::string_view HPWHSimInput::number_of_nodes_description =
    "Number of tank nodes used for simulation";
...
const std::string_view HPWHSimInput::depresses_temperature_description =
...
const std::string_view HPWHSimInput::fixed_volume_description =
    "Tank is not allowed to change volume";
...
const std::string_view HPWHSimInput::integrated_system_description =
    "The corresponding Standard 205 integrated water heater system";
...
const std::string_view HPWHSimInput::central_system_description =
    "The corresponding data group containing Standard 205 system component representations";
