...
{
    json_get<rsresistancewaterheatsource::ProductInformation>(j,
                                                              logger.get(),
                                                              "product_information",
...
void from_json(const nlohmann::json& j, RSRESISTANCEWATERHEATSOURCE& x)
{
    ...
    json_get<rsresistancewaterheatsource::Description>(
        j, logger.get(), "description", x.description, x.description_is_set, false);
    json_get<rsresistancewaterheatsource::Performance>(
