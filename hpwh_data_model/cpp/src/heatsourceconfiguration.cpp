...
{
    json_get<heat_source_configuration_ns::HeatingLogicType>(j,
                                                             logger.get(),
                                                             "heating_logic_type",
                                                             x.heating_logic_type,
                                                             x.heating_logic_type_is_set,
                                                             true);
    if (x.heating_logic_type ==
        heat_source_configuration_ns::HeatingLogicType::STATE_OF_CHARGE_BASED)
    {
        x.heating_logic =
            std::make_unique<heat_source_configuration_ns::StateOfChargeBasedHeatingLogic>();
        if (x.heating_logic)
        {
            from_json(j.at("heating_logic"),
                      *dynamic_cast<heat_source_configuration_ns::StateOfChargeBasedHeatingLogic*>(
                          x.heating_logic.get()));
        }
    }
    ...
    {
        x.heating_logic =
            std::make_unique<heat_source_configuration_ns::TemperatureBasedHeatingLogic>();
        if (x.heating_logic)
        {
            from_json(j.at("heating_logic"),
                      *dynamic_cast<heat_source_configuration_ns::TemperatureBasedHeatingLogic*>(
                          x.heating_logic.get()));
        }
    }
    json_get<heat_source_configuration_ns::ComparisonType>(
...
void from_json(const nlohmann::json& j, HeatSourceConfiguration& x)
{
    ...
    json_get<heat_source_configuration_ns::HeatSourceType>(
    ...
    json_get<heat_source_configuration_ns::WeightedDistribution>(j,
                                                                 logger.get(),
                                                                 "heat_distribution",
                                                                 x.heat_distribution,
                                                                 x.heat_distribution_is_set,
                                                                 true);
    json_get<std::vector<heat_source_configuration_ns::HeatingLogic>>(
    ...
    json_get<heat_source_configuration_ns::HeatingLogic>(
...
void from_json(const nlohmann::json& j, TemperatureBasedHeatingLogic& x)
{
    ...
    json_get<double>(j,
                     ...
                     true);
    json_get<heat_source_configuration_ns::StandbyTemperatureLocation>(
        j,
        logger.get(),
        ...
        true);
    json_get<heat_source_configuration_ns::WeightedDistribution>(
        j,
        logger.get(),
