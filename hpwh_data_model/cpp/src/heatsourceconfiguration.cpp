...
void from_json(const nlohmann::json& j, HeatingLogic& x) {
    json_get<heat_source_configuration_ns::HeatingLogicType>(j, logger.get(), "heating_logic_type", x.heating_logic_type, x.heating_logic_type_is_set, true);
    if (x.heating_logic_type == heat_source_configuration_ns::HeatingLogicType::STATE_OF_CHARGE_BASED) {
        x.heating_logic = std::make_unique<heat_source_configuration_ns::StateOfChargeBasedHeatingLogic>();
        if (x.heating_logic) {
            from_json(j.at("heating_logic"), *dynamic_cast<heat_source_configuration_ns::StateOfChargeBasedHeatingLogic*>(x.heating_logic.get()));
        }
    }
    if (x.heating_logic_type == heat_source_configuration_ns::HeatingLogicType::TEMPERATURE_BASED) {
        x.heating_logic = std::make_unique<heat_source_configuration_ns::TemperatureBasedHeatingLogic>();
        if (x.heating_logic) {
            from_json(j.at("heating_logic"), *dynamic_cast<heat_source_configuration_ns::TemperatureBasedHeatingLogic*>(x.heating_logic.get()));
        }
    }
    json_get<heat_source_configuration_ns::ComparisonType>(j, logger.get(), "comparison_type", x.comparison_type, x.comparison_type_is_set, true);
}
...
void from_json(const nlohmann::json& j, HeatSourceConfiguration& x) {
    ...
    json_get<heat_source_configuration_ns::HeatSourceType>(j, logger.get(), "heat_source_type", x.heat_source_type, x.heat_source_type_is_set, true);
    if (x.heat_source_type == heat_source_configuration_ns::HeatSourceType::RESISTANCE) {
        x.heat_source = std::make_unique<rsresistancewaterheatsource_ns::RSRESISTANCEWATERHEATSOURCE>();
        if (x.heat_source) {
            from_json(j.at("heat_source"), *dynamic_cast<rsresistancewaterheatsource_ns::RSRESISTANCEWATERHEATSOURCE*>(x.heat_source.get()));
        }
    }
    if (x.heat_source_type == heat_source_configuration_ns::HeatSourceType::CONDENSER) {
        x.heat_source = std::make_unique<rscondenserwaterheatsource_ns::RSCONDENSERWATERHEATSOURCE>();
        if (x.heat_source) {
            from_json(j.at("heat_source"), *dynamic_cast<rscondenserwaterheatsource_ns::RSCONDENSERWATERHEATSOURCE*>(x.heat_source.get()));
        }
    }
    json_get<heat_source_configuration_ns::WeightedDistribution>(j, logger.get(), "heat_distribution", x.heat_distribution, x.heat_distribution_is_set, true);
    json_get<std::vector<heat_source_configuration_ns::HeatingLogic>>(j, logger.get(), "turn_on_logic", x.turn_on_logic, x.turn_on_logic_is_set, true);
    json_get<std::vector<heat_source_configuration_ns::HeatingLogic>>(j, logger.get(), "shut_off_logic", x.shut_off_logic, x.shut_off_logic_is_set, false);
    json_get<heat_source_configuration_ns::HeatingLogic>(j, logger.get(), "standby_logic", x.standby_logic, x.standby_logic_is_set, false);
