...
void from_json(const nlohmann::json& j, CentralWaterHeatingSystem& x)
{
    ...
    json_get<std::vector<heat_source_configuration_ns::HeatSourceConfiguration>>(
        j,
        logger.get(),
    ...
    json_get<double>(j,
                     logger.get(),
                     "multipass_flow_rate",
                     x.multipass_flow_rate,
                     x.multipass_flow_rate_is_set,
                     false);
    json_get<double>(j,
                     logger.get(),
                     "standard_setpoint",
                     x.standard_setpoint,
                     x.standard_setpoint_is_set,
                     false);
}
...
const std::string_view CentralWaterHeatingSystem::standard_setpoint_units = "";
...
const std::string_view CentralWaterHeatingSystem::multipass_flow_rate_description =
    "Flow rate, if multipass system";

const std::string_view CentralWaterHeatingSystem::standard_setpoint_description =
    "Standard setpoint";
...
const std::string_view CentralWaterHeatingSystem::standard_setpoint_name = "standard_setpoint";
