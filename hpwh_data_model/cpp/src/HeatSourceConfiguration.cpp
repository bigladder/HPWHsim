...
void from_json(const nlohmann::json& j, HeatingLogic& x)
{
    ...
    {
        x.heating_logic =
            std::make_unique<heat_source_configuration::StateOfChargeBasedHeatingLogic>();
        if (x.heating_logic)
        {
            from_json(j.at("heating_logic"),
                      *dynamic_cast<heat_source_configuration::StateOfChargeBasedHeatingLogic*>(
                          x.heating_logic.get()));
        }
    }
    ...
    {
        x.heating_logic =
            std::make_unique<heat_source_configuration::TemperatureBasedHeatingLogic>();
        if (x.heating_logic)
        {
            from_json(j.at("heating_logic"),
                      *dynamic_cast<heat_source_configuration::TemperatureBasedHeatingLogic*>(
                          x.heating_logic.get()));
        }
    }
    json_get<heat_source_configuration::ComparisonType>(
...
void from_json(const nlohmann::json& j, HeatSourceConfiguration& x)
{
    ...
    json_get<heat_source_configuration::HeatSourceType>(
        j, logger.get(), "heat_source_type", x.heat_source_type, x.heat_source_type_is_set, true);
    // json_get<namespace>(j, logger.get(), "heat_source", x.heat_source, x.heat_source_is_set,
    // true);

    if (x.heat_source_type == heat_source_configuration::HeatSourceType::RESISTANCE)
    {
        x.heat_source =
            std::make_unique<rsresistancewaterheatsource::RSRESISTANCEWATERHEATSOURCE>();
        if (x.heat_source)
        {
            from_json(j.at("heat_source"),
                      *dynamic_cast<rsresistancewaterheatsource::RSRESISTANCEWATERHEATSOURCE*>(
                          x.heat_source.get()));
        }
    }
    if (x.heat_source_type == heat_source_configuration::HeatSourceType::CONDENSER)
    {
        ...
        {
            from_json(j.at("heat_source"),
                      *dynamic_cast<rscondenserwaterheatsource::RSCONDENSERWATERHEATSOURCE*>(
                          x.heat_source.get()));
        }
    }
    if (x.heat_source_type == heat_source_configuration::HeatSourceType::AIRTOWATERHEATPUMP)
    {
        ...
        if (x.heat_source)
        {
            from_json(
                j.at("heat_source"),
                *dynamic_cast<rsairtowaterheatpump::RSAIRTOWATERHEATPUMP*>(x.heat_source.get()));
        }
    }
    json_get<heat_source_configuration::WeightedDistribution>(j,
                                                              logger.get(),
                                                              "heat_distribution",
                                                              x.heat_distribution,
                                                              x.heat_distribution_is_set,
                                                              true);
    json_get<std::vector<heat_source_configuration::HeatingLogic>>(
    ...
    json_get<heat_source_configuration::HeatingLogic>(
...
void from_json(const nlohmann::json& j, TemperatureBasedHeatingLogic& x)
{
    ...
    json_get<double>(j,
                     ...
                     true);
    json_get<heat_source_configuration::StandbyTemperatureLocation>(
        j,
        logger.get(),
        "standby_temperature_location",
        x.standby_temperature_location,
        x.standby_temperature_location_is_set,
        true);
    json_get<heat_source_configuration::WeightedDistribution>(
        j,
        logger.get(),
...
const std::string_view TemperatureBasedHeatingLogic::standby_temperature_location_description =
    "Standby temperature location";

const std::string_view TemperatureBasedHeatingLogic::standby_temperature_location_name =
    "standby_temperature_location";
