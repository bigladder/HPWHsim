...
void from_json(const nlohmann::json& j, HPWHSimInput& x)
{
    ...
    json_get<double>(j,
                     ...
                     false);
    json_get<hpwh_sim_input_ns::HPWHSystemType>(
        j, logger.get(), "system_type", x.system_type, x.system_type_is_set, true);
    json_get<rsintegratedwaterheater_ns::RSINTEGRATEDWATERHEATER>(j,
                                                                  logger.get(),
                                                                  "integrated_system",
                                                                  x.integrated_system,
                                                                  x.integrated_system_is_set,
                                                                  true);
    json_get<central_water_heating_system_ns::CentralWaterHeatingSystem>(
